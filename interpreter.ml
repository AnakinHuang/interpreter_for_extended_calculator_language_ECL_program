 ...
 *)
 ...
 let rec interpret (ast:ast_sl) (full_input:string) : string =
    (*  ast is the abstract syntax tree to be interpreted
        full_input is the entire input string, which is split into words
        and passed to interpret_sl *)
 ...
 (* Code below expects there to be a single command-line argument, which
   ...
   let read_prog () =
     if (Array.length Sys.argv) <> 2 (* <> replaced != for structural equality *)
